...
def generate_xs(sample_size: int = 200):
    return np.linspace(0, 1, sample_size + 1)[1:]


# class OracleModel:
#     def __init__(self, n_coef: int = 200, seed: Optional[int] = None, sigma: float = 1.0):
#         self.n_coef = n_coef
#         self.seed = seed
#         if self.seed:
...
#     @staticmethod
#     def sample_coefs(n_coef: int = 200) -> np.ndarray:
...
#     @staticmethod
#     def fourier_basis(xs: np.ndarray, n_coef: int = 200) -> np.ndarray:
...
#     @staticmethod
#     def grad_fourier_basis(xs: np.ndarray, n_coef: int = 200) -> np.ndarray:
#         evens = np.arange(2, n_coef+1, 2)
#         odds = np.arange(1, n_coef+1, 2)
#         psi = np.empty(list(xs.shape) + [n_coef])
#         psi[..., evens-1] = -np.sin(np.pi * xs[..., None] * evens[None, :]) * np.pi * evens[..., :]
#         psi[..., odds-1] = np.cos(np.pi * xs[..., None] * (odds[None, :] + 1)) * np.pi * odds[..., :]
...
class DataModel:
    def __init__(
        self, n_coef: int = 200, seed: Optional[int] = None, sigma: float = 1.0
    ):
        self.n_coef = n_coef
        self.seed = seed
        if self.seed:
